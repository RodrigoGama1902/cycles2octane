...
def ShaderNodeOctMixTex(new_node, old_node):
    ...
    def replace_mix_operation(mix_node, to_operation):
        ...
        new_op_node.inputs[0].default_value = get_correct_value(new_op_node.inputs[0], old_node.inputs[1].default_value)
        new_op_node.inputs[1].default_value = get_correct_value(new_op_node.inputs[0], old_node.inputs[2].default_value)
                
        link = node_tree.links.new    
        
        if mix_node.inputs[1].links:     
            link(mix_node.inputs[1].links[0].from_socket, new_op_node.inputs[0])
        if mix_node.inputs[2].links: 
            link(mix_node.inputs[2].links[0].from_socket, new_op_node.inputs[1]) 
...
def ShaderNodeOctAddTex(new_node, old_node):
    ...
    def replace_math_operation(math_node, to_operation):
        ...
        new_op_node.inputs[0].default_value = get_correct_value(new_op_node.inputs[0], old_node.inputs[0].default_value)
        new_op_node.inputs[1].default_value = get_correct_value(new_op_node.inputs[0], old_node.inputs[1].default_value)
                
        link = node_tree.links.new    
        
        if math_node.inputs[0].links:     
            link(math_node.inputs[0].links[0].from_socket, new_op_node.inputs[0])
        if math_node.inputs[1].links: 
            link(math_node.inputs[1].links[0].from_socket, new_op_node.inputs[1]) 
            ...
            for i in new_node.outputs["Normal"].links:
                if i.to_node.type == "OCT_UNIVERSAL_MAT":
                    if i.to_socket.name == "Normal":
                        ...
                        link = node_tree.links.new
                        link(new_node.outputs["Bump"], i.to_node.inputs["Bump"])
