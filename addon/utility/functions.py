...
def node_replacer(node):
    ...
    def in_out_mng(input=True):
        ...
        for i in node_for:         
            ...
            if not node_index:              
                replace_in_out = in_out_dict[key_name] if in_out_dict.get(key_name) else None
            else:
                try:                
            ...
            if replace_in_out:
                ...
                if hasattr(i,'default_value') and hasattr(new_node_in_out[replace_in_out], "default_value"):
                    ...
                    try:
                        new_node_in_out[replace_in_out].default_value = get_correct_value(new_node_in_out[replace_in_out], i.default_value)                                           
                    except ValueError:
                        pass
                    except TypeError:                      
                        pass
                ...
                if i.links:
                    for link_s in i.links:
                        if input:
                            link(new_node.inputs[replace_in_out], link_s.from_socket)  
                        else:
    ...
    null_links = {}
    null_node = False  # This variable tells if the node that will replace the current one will be a null node
    group_inputs = []
    group_outputs = []
    ...
    if hasattr(cycles2octane_pre_functions, node.bl_idname if not "NULL_NODE_" in node.name else node.name):
        ...
        if not "NULL_NODE_" in node.name:
            node_pre_function = getattr(cycles2octane_pre_functions, node.bl_idname, False)
        else:
            node_pre_function = getattr(cycles2octane_pre_functions, node.name, False)
    ...
    if null_node:                      
        new_node = create_null_node(node, node_tree, null_links, group_inputs, group_outputs)
    else:    
        if convert_to_node:                          
    ...
    node_index = False  
    if replace_inputs:           
        node_index = True if [f for f in replace_inputs][0].isdigit() else False
    ...
    if hasattr(cycles2octane_post_functions, new_node.bl_idname if not null_node else new_node.name):
        ...
        if not null_node:
            node_function = getattr(cycles2octane_post_functions, new_node.bl_idname, False)
        else:
            node_function = getattr(cycles2octane_post_functions, new_node.name, False)
...
def create_null_node(node, node_tree, null_links, group_inputs, group_outputs):
    ...
    group_tree = bpy.data.node_groups.new("NULL_NODE_" + node.bl_idname, 'ShaderNodeTree')
